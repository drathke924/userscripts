...
def partone(datain):
		...
		while True:
			...
			if instruct[0] == "set":
				...
				try:
					...
					instruct[1] = regset[instruct[1]]
				return ("snd", instruct[1], regset, step)
			elif instruct[0] == "rcv":
				return ("rcv", instruct[1], regset, step)
			elif instruct[0] == "jgz":
				try:
					instruct[1] = int(instruct[1])
				except ValueError:
	...
	def processone(datain, stackone, step):
		next = genericprocess(step)
	
	def processtwo(datain, stacktwo, step):
